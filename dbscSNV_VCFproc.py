...
def filter_scsnv_by_exome_list(chrom_db, bed_chrom_dict):
    ...
    chrom_db_filtered = chrom_db[chrom_db.POS.isin(create_exome_list(bed_chrom_dict))]
...
def format_to_VCF(chrom_preformat):
    ...
    chrom_final = comp_ada_rf_scores(chrom_preformat)  #


    chrom_final["CHROM"] = "chr" + str(chrom_final.CHROM[1])
    
    chrom_final.insert(2, "ID", '.')
    chrom_final.drop(["ada_score","rf_score","splice_alt"], axis=1, inplace=True)
...
def write_flat_file_dbscSNV(chrom_final_format, chrom_num):
    ...
    chrom_final_format.to_csv("dbscSNV_"+str(chrom_num)+".vcf", sep='\t', index=False)
...
def main():
    ...
    for i in range(len(snv_list)):
        write_flat_file_dbscSNV(format_to_VCF(snv_list[i]), snv_list[i].CHROM[1])
